...
type Session struct {
	Path		string	`json:"path"` // wallet file path
	Password	string	`json:"password"` // password
	Mode		string	`json:"mode"` // network mode (remote, local, offline)
	Syncing		bool	`json:"syncing"` // sync status
	Network		string	`json:"network"` // Testnet or Mainnet
	Daemon		string	`json:"daemon"` // current daemon address
	Rescan		bool	`json:"rescan"` // trigger rescan
	RPCServer	bool	`json:"rpcserver"` // toggle RPC server
	RPCAddress	string
	Color		rl.Color
}
...
func main() {
	...
	for !rl.WindowShouldClose() {
		...
		if loadScreenColor.A == 0 {
			...
			if (windowIndex != 2.2 && windowIndex != 2.3 && windowIndex != 2.4 && windowIndex != 2.5 && windowIndex != 2.51 && windowIndex != 2.52 && windowIndex != 2.53 && windowIndex != 2.54 && windowIndex != 2.6) {
			...
			if rl.IsKeyReleased(rl.KeyGrave) {
			...
			case 2.2:
				if session.Path == "" {
					...
					rl.DrawTextEx(font, "Welcome CMDR, here you can view the status of your active account, manage application settings, and more.", rl.Vector2{offsetX, 80}, 20, spacing, rl.Gray)
			...
			case 2.4:
				...
				rl.DrawTextEx(fontHeader, "Public Account Address", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 2.5:
				...
				rl.DrawTextEx(fontHeader, "Destination Address:  (Ctrl-V to paste)", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 2.51:
				...
				rl.DrawTextEx(fontHeader, "Payment ID (optional):", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 2.52:
				...
				rl.DrawTextEx(fontHeader, "Amount:", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 2.53:
				...
				rl.DrawTextEx(fontHeader, "My Password:", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 3.31:
				...
				rl.DrawTextEx(fontMenu, "Restore an Account", rl.Vector2{offsetX, 30}, 40, spacing, rl.Gray)
				rl.DrawTextEx(font, "Restore using Hex (64 chars)", rl.Vector2{offsetX, 80}, 20, spacing, rl.Gray)
				rl.DrawTextEx(fontHeader, "Account Password:", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
				_, restorePassword = rl.GuiTextBox(rl.NewRectangle(offsetX, 220, 400, 50), restorePassword, 160, restorePasswordEditable)
				rl.DrawRectangleRec(rl.NewRectangle(offsetX, 215, 500, 60), cmdGray)
				rl.DrawLineEx(rl.Vector2{offsetX, 275}, rl.Vector2{offsetX + 500, 275}, 2.0, session.Color)
			...
			case 3.32:
				rl.DrawTextEx(fontMenu, "Restore an Account", rl.Vector2{offsetX, 30}, 40, spacing, rl.Gray)
				rl.DrawTextEx(font, "Restore using Hex (64 chars)", rl.Vector2{offsetX, 80}, 20, spacing, rl.Gray)
				rl.DrawTextEx(fontHeader, "Confirm Password:", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
				_, tempPassword = rl.GuiTextBox(rl.NewRectangle(offsetX, 220, 400, 50), tempPassword, 160, tempPasswordEditable)
				rl.DrawRectangleRec(rl.NewRectangle(offsetX, 215, 500, 60), cmdGray)
				rl.DrawLineEx(rl.Vector2{offsetX, 275}, rl.Vector2{offsetX + 500, 275}, 2.0, session.Color)
			...
			case 3.33:
				rl.DrawTextEx(fontMenu, "Restore an Account", rl.Vector2{offsetX, 30}, 40, spacing, rl.Gray)
				rl.DrawTextEx(font, "Restore using Hex (64 chars)", rl.Vector2{offsetX, 80}, 20, spacing, rl.Gray)
				rl.DrawTextEx(fontHeader, "Enter your 65 hex chars (Ctrl-V to Paste) :", rl.Vector2{offsetX, 150}, 30, spacing, rl.White)
			...
			case 4.0:
				clearVars()
				rl.DrawTextEx(fontHeader, "Settings have been disabled in this release.", rl.Vector2{offsetX, 200}, 30, spacing, rl.Orange)
				break
