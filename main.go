	...
	for !rl.WindowShouldClose() {
		...
		if loadScreenColor.A == 0 {
			...
			case 4.2:
				...
				session.Network = modeList[active]
			...
			case 4.3:
				...
				_, config.RemoteDaemon = rl.GuiTextBox(rl.NewRectangle(offsetX, 220, 400, 50), config.RemoteDaemon, 160, em)
...
func reloadConfig() {
	session.Network = config.DefaultNetwork
	session.Mode = config.DefaultMode
	session.Rescan = false
	session.Syncing = false
	session.RPCServer = false
